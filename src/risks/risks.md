...
Forward-looking risks only. Descriptions use "may / could" on purpose — if wording turns to present tense ("is happening"), the item belongs in the [Issue Log](#issue-log-occurred-problems), not here. Triggers are **leading indicators** (early-warning signs) rather than the problem itself.
