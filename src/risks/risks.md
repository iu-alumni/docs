...
| R-02 | Infrastructure | Database migration fails due to missing or corrupted backup → partial data loss, 4+ h restore, delays | High | Medium | High | **Resolved** (did not occur) | The database migration was executed safely — pre-migration backup + staging dry-run + row-count and referential-integrity checks all passed ([architecture design §4.1–4.2](../project/architecture_design.md)), no data loss. The platform currently runs on the Yandex servers; the planned move to the Innopolis university servers (the end goal) is still pending, blocked by server-side issues on the DoE side. Ongoing data safety is covered by tiered Postgres backups | Ahmad Helaly | 13.04.2026 | 10.05.2026 |
