# Sprint 0 — Team Meeting 1

**Date:** February 14th, 2026, 19:00
**Attendees:** Roukaya Mohammed · Majed Naser · Ahmad Helaly · Ghadeer Akleh · Aleksandr Kovalev

## Agenda

1. Roles — who does what and what are the responsibilities for each member
2. Organizing work for IP
3. Identify goals for the next period and what needs to be achieved

---

## Documentation

- **GitHub Documentation:** Managed by Roukaya (primary responsibility)
- **Google Drive:** Contains meeting recordings and presentations

---

## Backlog Items

- **User Registration:** Add personal email field or an additional verification method to facilitate password recovery

---

## Rustore Deployment

Currently only functional through GitFlick.

**Proposed solutions:**

1. Mirror solution _(preferred approach)_
2. API request integration with Rustore

---

## Open Questions

### For the Client

- What is the current user validation process?

### For the IT Team

- **Access requirements:**
  - Database access or SSO integration
  - SSH accessibility outside the university network
- **Infrastructure requests:**
  - Upgrade testing machine resources to match the production environment
  - Request subdomains for backend services, Grafana monitoring, and log management

---

## 👥 Team Roles & Responsibilities

| Team Member           | Responsibilities       |
| --------------------- | ---------------------- |
| **Roukaya Mohammed**  | Meeting facilitator    |
| **Ahmad Helaly**      | TBD                    |
| **Majed Naser**       | TBD                    |
| **Ghadeer Akleh**     | Meeting recordings     |
| **Aleksandr Kovalev** | TBD                    |

> **Note:** Full role assignments are pending — to be confirmed in the next meeting.
